...
def test_bad_config(ddqa, mocker):
    ...
    assert install_screen.call_args_list == [mocker.call(sync_screen, 'sync')]
...
def test_valid_setup(ddqa, isolation, config_file, mocker):
    ...
    install_screen.assert_called_once_with(sync_screen, 'sync')
    push_screen.assert_called_once_with('sync')
